...
kb_thread_pool* kb_threadpool_create(int num_threads) {
  ...
  if (pool == NULL) {
    kb_log_error("Failed to create thread pool");
    return pool;
  }
  ...
  if (jobqueue_init(&pool->jobqueue) == -1){
    kb_log_error("kb_threadpool_create(): Could not allocate memory for job queue\n");
...
int kb_threadpool_add_job(kb_thread_pool* pool, void* userdata, kb_job_func func) {
  ...
  if (job == NULL){
    kb_log_error("kb_threadpool_add_job(): Could not allocate memory for new job\n");
    return -1;
  }
